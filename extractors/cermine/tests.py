"""Test the Dockerized CERMINE extractor application."""
...
class TestCERMINEExtractor(TestCase):
    """Build, run, and test the Dockerized CERMINE extractor."""

    @classmethod
    def setUpClass(cls):
        """Build the docker image and start it."""
        build = subprocess.run(
            "docker build . -t arxiv/cermine",
        ...
        start = subprocess.run(
            "docker run -d -p 8912:8000 arxiv/cermine",
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            shell=True, cwd=WORKDIR
        )
        ...
        print(f'Started CERMINE extract as {cls.container}')
    ...
    def test_extract(self):
        ...
        with open(pdf_path, 'rb') as f:
            response = requests.post('http://localhost:8912/cermine/extract',
                                     files={'file': f})
        ...
        try:
            ...
            self.fail("Should return valid XML")
